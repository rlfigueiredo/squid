...
funcconfsquid () {
	...
	zipsquid="$raizinstall/install/$versaosquid.tar.bz2"
	bincalamaris="$raizinstall/install/$versaocalamaris/calamaris"
	...
	cp $raizinstall/squid/squid.conf.padrao $raizinstall/squid/squid.conf.$cliente
	...
	cp $raizinstall/squid/squid.conf.$cliente $squidconf
	$squidsbin -z
...
funcconfsquidguard () {
	...
	echo "## declaracao de variaveis"
	zipsquidGuard="$raizinstall/install/$versaosquidGuard.tar.gz"
...
funcconfapache () {
	...
	apacheconfnovo="$raizinstall/apache/conf/httpd.conf.$cliente"
	apacheconftemp="$raizinstall/apache/conf/httpd.conf.temp"
	paginarestart="/srv/httpd/htdocs/admin/proxy/restart/restart.php"
	paginarestartnova="$raizinstall/apache/restart.php.nova"
	paginasquidguard="/srv/httpd/htdocs/erro/squidGuard.php"
	paginasquidguardnova="$raizinstall/apache/squidGuard.php.nova"
	
	
	echo "## copiando arquivos de configuracao"
	cp -r $raizinstall/apache/htdocs/* /srv/httpd/htdocs
	cp $apachehost $apachehost.orig
	cp $apachehost $raizinstall/apache/conf/

	echo "## acertando arquivos de configuracao"
	sed "s+index.html+index.html index.php+g" $raizinstall/apache/conf/httpd.conf >$apacheconftemp
	sed "s+#Include /etc/httpd/mod_php.conf+Include /etc/httpd/mod_php.conf+g" $apacheconftemp >$apacheconfnovo
	...
	cp $paginarestart $raizinstall/apache/
	sed "s+http://localhost+http://$subnet.1+g" $raizinstall/apache/restart.php >$paginarestartnova
	cp $paginarestartnova $paginarestart
	cp $paginasquidguard $raizinstall/apache/
	sed "s+squidclientesolutionlinux+$email+g" $raizinstall/apache/squidGuard.php >$paginasquidguardnova
...
funcconfusuario () {
	...
	cat $raizinstall/conf_so/shadow-admroot >>$shadow
	mkdir $home $ssh_dir
	cp $raizinstall/conf_so/authorized_keys $ssh_key
...
funcconfso () {
	...
	sshd_novo="$raizinstall/conf_so/sshd_config.novo"
	lilo="/etc/lilo.conf"
	lilo_novo="$raizinstall/conf_so/lilo.conf.novo"
	inittab="/etc/inittab"
	inittab_novo="$raizinstall/conf_so/inittab.novo"
	...
	cp $sshd $raizinstall/conf_so
	sed "s+#PermitRootLogin yes+PermitRootLogin no+g" $raizinstall/conf_so/sshd_config >$sshd_novo
	cp $sshd_novo $sshd

	echo "## acertando lilo.conf"
	cp $lilo $raizinstall/conf_so
	sed "s+timeout = 1200+timeout = 12+g" $raizinstall/conf_so/lilo.conf >$lilo_novo
	cp $lilo_novo $lilo
	lilo

	echo "## acertando inittab"
	cp $inittab $raizinstall/conf_so
	sed "s+ca::ctrlaltdel:/sbin/shutdown -t5 -r now+#ca::ctrlaltdel:/sbin/shutdown -t5 -r now+g" $raizinstall/conf_so/inittab >$inittab_novo
	cp $inittab_novo $inittab
	telinit q

	echo "## copiando demais arquivos novos"
	cp $raizinstall/conf_so/logrotate.squid $logrotate_squid
	cp $raizinstall/conf_so/logrotate.syslog $logrotate_syslog
	cp $raizinstall/conf_so/logrotate.httpd $logrotate_httpd
	cp $raizinstall/conf_so/rc.local $rc_local
	cp $raizinstall/conf_so/sudoers $sudoers
	cp $raizinstall/conf_so/syslog.conf $syslog
	cp $raizinstall/conf_so/inetd.conf $inetd
	cp -r $raizinstall/scripts/ /root

	echo "## acertando crontab"
	cat $raizinstall/scripts/crontab >>$crontab
...
funcconffirewall () {
	...
	fwin_pad="$raizinstall/firewall/rc.firewall.padrao"
	fwin_final="$raizinstall/firewall/rc.firewall.$cliente.$data"
...
funcconfdhcp () {
	...
	dhcp_conf="$raizinstall/conf_so/dhcpd.conf.$cliente"
...
funcconfhorario() {
	...
	cp $raizinstall/conf_so/$arquivozic /root/scripts
...
funcconfnetsnmp () {
	...
	echo "## declarando variavies ##"
	zipnetsnmp="$raizinstall/install/$versaonetsnmp.tar.gz"
	...
	snmpconf_novo="$raizinstall/conf_so/snmpd.conf"
...
funcconfgd() {
	...
	echo "## declarando variavies ##"
	zipgd="$raizinstall/install/$versaogd.tar.bz2"
...
funcconfmrtg() {
	...
	echo "## declarando variavies ##"
	zipmrtg="$raizinstall/install/$versaomrtg.tar.gz"
...
funcconfntop () {
	...
	echo "## declarando variavies ##"
	zipntop="$raizinstall/install/$versaontop.tgz"
...
funcconfbd () {
	...
	echo "## declarando variavies ##"
	zipbd="$raizinstall/install/$versaobd.tar.gz"
...
if [ -z $2 ];then  
	echo "Digitar: $0 cliente raiz_install"
	echo "Exemplo: $0 solucont /tmp/squid"
	exit 0
fi
#host="$1"
cliente="$1"
