...
system ("echo '	urllist		blacklist/urls' >>$squidguard");
...
foreach ( $todos as $acls => $acl ) {
	...
	if ( $acl <> ""  && $acl <> "blacklist" && $acl <> "todos") {
		system ("echo '	$acl {' >>$squidguard");
		system ("echo '		pass !lista_$acl lista_todos !blacklist all' >>$squidguard");
		system ("echo '	}' >>$squidguard");
	}
}
system ("echo '	default {' >>$squidguard");
system ("echo '		pass !blacklist lista_todos none' >>$squidguard");
system ("echo '		redirect http://localhost/erro/squidGuard.php?clientaddr=%a&clientname=%n&clientident=%i&clientgroup=%s&destinationgroup=%t&url=%u' >>$squidguard");
##system ("echo '		redirect http://rogerio.ultraespaco.com' >>$squidguard");
system ("echo '	}' >>$squidguard");
system ("echo '}' >>$squidguard");
